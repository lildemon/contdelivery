...
extend(Project.prototype, {
	...
	installWebpack: function(router) {
		...
		this.db.findOne({type: 'wpconf'}, function(err, doc) {
			...
			// update project path incase it's diffent
			self.wpconf.projectPath = self.path 
			self.wpconf.wwwPath = path.join(self.path, self.workingDirName)
	...
	loadProjectRoute: function(router) {
		...
		try {
			fs.readdirSync(path.join(this.path, this.infDirName, 'routes')).forEach(function(file) {
				...
				loadingFile = file
				require(path.join(self.path, self.infDirName, file))(router)
			})
		} catch (e) {
