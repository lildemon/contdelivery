...
extend(Project.prototype, {
	...
	updateMiddleware: function() {
		...
		this.loadAdditionalStatic(router)
	...
	loadAdditionalStatic: function(router) {
		var dirDef = this.getConfig('dirs')
		var virtualDir, serveDir
		for(virtualDir in dirDef) {
			serveDir = dirDef[virtualDir]
			router.use('/' + virtualDir, express.static(serveDir))
		}
	},
