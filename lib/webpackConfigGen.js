...
module.exports = function(opt, project) {
	...
	var srvWpOpt = {
		...
		output: {
			...
			pathinfo: !webpack.PRODUCTION //实时编译环境在.map文件里输出文件路径信息
		},
		amd: { jQuery: true },
		resolve: {
		...
		module: {
			loaders: [
				...
				{
					test: /\.jsx$/,
					loader: opt.es6 ? 'babel-loader?experimental&optional=runtime' : 'jsx-loader'
				},
	...
	try {
		...
		if(wpConfig) {
			_.merge(srvWpOpt, wpConfig, function(a, b) {
				if (_.isArray(a)) {
					// 数组的话用追加方式
				    return a.concat(b);
				  }
